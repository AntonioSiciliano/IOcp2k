...
# Hardcoded variable in ANGSTROM (DO NOT TOUCH IT)
...
def get_types_idx(atoms, my_types = ["Na", "Cl"], excluded_position = EXCLUDED_POS, debug = False):
    ...
    -----------
        ...
        -excluded_pos: np.array of shape (3,), atoms in this positions will be removed
        -debug: bool
    ...
    if not excluded_position is None:
        ...
        if rank == 0 and debug:
            ...
            if np.einsum('ab -> b', atoms.positions[idx_to_exclude_0,:] - excluded_position).sum() > 1e-3:
                raise ValueError("Excluding wrong atoms 0")

            if np.einsum('ab -> b', atoms.positions[idx_to_exclude_1,:] - excluded_position).sum() > 1e-3:
                raise ValueError("Excluding wrong atoms 1")
    
        # Get rid of the atoms that are in excluded_position
...
def cluster_analysis(atoms, index = 0, 
    ...
    ----------
        ...
        -index: int, the id of atoms object for printing pourposes
        ...
        -indices_types: list of np.array of len (Nat_1,) and (Nat_2,), the indices of the atoms of types
    ...
    """
    # Identify at1 and at2 indices using the list
    ...
    if rank==0 and debug:
        print("\nCLUSTER | Atom {} #{} indices {}".format(types[0], len(sel_types_idx_0), sel_types_idx_0))
        print("CLUSTER | Atom {} #{} indices {}".format(types[1], len(sel_types_idx_1), sel_types_idx_1))
        ...
        for j, at1 in enumerate(sel_types_idx_1):
            if adjacency[i, j]:
                ...
                if rank==0 and debug:
                    ...
                    print("CLUSTER | Graph edge {} d {:.4f} Ang".format(edge, atoms.get_distance(at0, at1, mic = True)))
    
    # Find clusters (connected components)
    clusters = list(nx.connected_components(G))
    if rank==0 and debug:
        print()
        print(f"CLUSTER | Number of {types[0]}{types[1]} clusters: {len(clusters)}\n")
    ...
    # Cluster is of type set
    for i, cluster in enumerate(clusters):
        if rank == 0 and debug:
            print("CLUSTER | We analyze the cluster ", cluster)
        ...
        # Get the composition
        # element contains the atom type and the atomic ids
        for element in cluster:
            # print(element)
            # GET THE ID OF THE ATOMS FORMING THE CLUSTERS
        ...
        if rank==0 and debug:
            print(f"CLUSTER | Cluster #{i} with {len(cluster)} atoms")
            print(f"CLUSTER | and composition {composition}")
            print(f"CLUSTER | {types[0]} | ids {at0_ind}")
            print(f"CLUSTER | {types[1]} | ids {at1_ind}")
            print(f"CLUSTER | {types[0]} {types[1]} | ids {all_at0_at1_ind[-1]}")
            # Cycle on the atomic indicices of the cluster to check connectivity
            for i in at0_ind:
                for j in at1_ind:
                    print("CLUSTER | {}[{}] {}[{}] d {:.3f} Ang".format(types[0], i, types[1], j, atoms.get_distance(i, j, mic = True)))
            print()


    if rank==0 and debug:
        print("CLUSTER | The ids of the atoms forming all the cluster")
        for ind, item in enumerate(all_at0_at1_ind):
            print("CLUSTER #{} {}".format(ind, item))
        print()
        
    return len(clusters), np.asarray(all_cluster_size), all_cluster_composition, all_at0_ind, all_at1_ind, all_at0_at1_ind


def HEAVY_get_dist_mat(atom, ids0, ids1):
    """
    DEBUG
    =====
    """
    d = np.zeros((len(ids0), len(ids1)))

    for _i, i in enumerate(ids0):
        for _j, j in enumerate(ids1):
            d[_i,_j] = atom.get_distance(i,j,mic = True)
    return d
...
def get_mask(atom, ids0, ids1, min_r, max_r, debug = False):
    ...
    and define an adjacency mask (EXTREMES ARE INCLUDED)
    ...
    if rank == 0 and debug:
        print("\n  MASK | -GET ADJACENCY MATRIX-")
        print("  MASK | Type 1 ids", ids0)
        print("  MASK | Type 2 ids", ids1)
        # print("  MASK | DIST MAT [ANG]")
        # print(d)
        # print()

        # HEAVY TEST
        _d_ = HEAVY_get_dist_mat(atom, ids0, ids1)
        if np.abs(np.sum((_d_ - d).ravel())) > 1e-10:
            raise ValueError("The distance matrix is WRONG")
...
def look_for_clusters(configs, atoms, min_r = 0.9, max_r = 1.0, my_types = ["Na", "Cl"], verbose = True, debug = False):
    ...
    for i, atom in enumerate(atoms):
        ...
        if rank == 0 and ((verbose and not debug and configs[i] % PRINT_EVERY_N == 0) or (verbose and debug and configs[i] % 1 == 0)):
        ...
        mask_d = get_mask(atom, sel_types_idx_0, sel_types_idx_1, min_r, max_r, debug = debug)

        if rank == 0 and ((verbose and not debug and configs[i] % PRINT_EVERY_N == 0) or (verbose and debug and configs[i] % 1 == 0)):
            print("\n---------------------------------------")
            print("  ELAPSED TIME {:.2f} sec".format(time.time() - t1))
        ...
        # num_clusters is a int, the number of clusters
        # sizes is the size of each clusters
        # compositiosn is a list of strings containing the compositon of each cluster eg ["A1B2", "A2B3", ..]
        # at0_inds list with the indices of the atoms forming each cluster
        # at1_inds list with the indices of the atoms forming each cluster
        # at0_at1_inds list with the indices of both atom tpyes forming each cluster
...
if __name__ == '__main__':
    ...
    We look for clusters of atoms with a pairwise distance of [rc_min, rc_max] Angstrom (lower and upper bounds are included)
    ...
    print("RANK {} will compute {} configurations from #{} to #{}\n\n\n".format(rank, len(local_configs),
    ...
    print("\n\nRANK {} HAS FOUND {} CONFIGURATIONS BONDED".format(rank, len(local_results)))
