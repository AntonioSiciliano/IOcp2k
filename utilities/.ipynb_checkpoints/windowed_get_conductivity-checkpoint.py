...
if __name__ == '__main__':
    ...
    all_ase_atoms = all_ase_atoms[:300]
    ...
    JJ       = np.zeros(len_full_trajectory, dtype = float)
    error_JJ = np.zeros(len_full_trajectory, dtype = float)
    ...
    for m in np.arange(len_full_trajectory):
        ...
        mean, error = get_JJ(m, len_full_trajectory, selected_atoms, selected_qs)
        JJ[m] = mean
        error_JJ[m] = error
        # Get the average volume ANGSTROM^3
        V_mean += all_ase_atoms[m].get_volume()

    V_mean /= len_full_trajectory
    ...
    save_plot_results(t, JJ, error_JJ)